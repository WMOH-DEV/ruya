...
@section('content')
    <div class="container-fluid">
        ...
        <div class="content-header">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark">في إنتظار الموافقة</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item">الرئيسية</li>
                        <li class="breadcrumb-item">المُعلمين</li>
                        <li class="breadcrumb-item active">في إنتظار الموافقة</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div>
                ...
                <div class="table-responsive">
                    ...
                    <table class="table table-hover text-center" id="myTable">
                        ...
                        @foreach($teachers as $teacher)
                            ...
                            <tr>
                                <td class="align-middle"><img class="rounded" style="height: 4rem;"
                                                              src="{{asset('uploads')}}/{{$teacher->profile_img}}"
                                                              alt=""></td>
                                <td class="align-middle">
                                    <a href="{{url('teachers/show')}}/{{$teacher->id}}" target="_blank" style="text-decoration: none;">
                                    {{$teacher->user->fullName()}}
                                    </a>
                                </td>
                                <td class="align-middle">{{$teacher->user->email}}</td>
                                <td class="align-middle">{{$teacher->user->country->country_name}}</td>
                                <td class="align-middle">@if($teacher->user->gender == 'male')
                                        مُعلم@elseif($teacher->user->gender == 'female') معلمة @endif</td>
                                <td class="align-middle">{{$teacher->qualification}}</td>
                                <td class="align-middle">{{$teacher->experience}}</td>
                                <td class="align-middle">@if(isset($teacher->subject->subject_name)) {{$teacher->subject->subject_name}} @else
                                        <span class="badge badge-dark">مادة محذوفة</span> @endif</td>
                                <td class="align-middle">@if($teacher->other_subjects) {{$teacher->other_subjects}} @else
                                        <span class="badge badge-light"> لا يُدرس مواد آخرى</span> @endif
                                <td class="align-middle" >
                                ...
                                <td class="align-middle">
                                    ...
                                    <button class="btn btn-primary py-1 px-2"  data-toggle="modal"
                                            data-target="#accept{{$teacher->id}}" data-toggle="tooltip" data-placement="top"
                                            title="الموافقة على المُعلم">
                                        <i class="fa fa-check"></i>
                                    </button>

                                    <div class="modal fade" id="accept{{$teacher->id}}" tabindex="-1" role="dialog"
                                         aria-labelledby="exampleModalLabel" aria-hidden="true">
                                        <div class="modal-dialog" role="document" >
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="exampleModalLabel"> الموافقة على المعلم</h5>
                                                    <button type="button" class="close" data-dismiss="modal"
                                                            aria-label="Close">
                                                        <span aria-hidden="true">&times;</span>
                                                    </button>
                                                </div>
                                                <form action="{{url('admincp/teachers/accept')}}" method="post">
                                                    @csrf
                                                    <div class="modal-body">
                                                        <input type="text" hidden value="{{$teacher->id}}" name="teacher_id">

                                                        <p> الموافقة على المعلم وعرضه في نتائج بحث الموقع ؟</p>
                                                            <span class="badge badge-primary">سيتم إخطار المعلم عبر رسالة بريدية</span>
...
@section('custom-js')
    ...
    <script src="{{asset('cp/js/draggable.js')}}"></script>
    <script src="{{asset('cp/js/EZView.js')}}"></script>
