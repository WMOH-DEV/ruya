...
@section('content')
    <div
        class="teacher-container px-5  md:px-10  md:px-14 pt-16 lg:pt-32 min-h-screen min-h-screen flex flex-col justify-between">
...
{{--        @endif--}}
        <!-- content -->
            <form class="grid grid-cols-1 md:grid-cols-4 gap-4 mx-auto px-5 md:mt-5 " method="get"
                  action="{{url('search')}}">
                @csrf
                <label>
                    <select style="background-color:#fff;"
                            id="stages"
                            class="form-control w-full appearance-none border-gray-200 rounded border shadow-sm p-3 my-1 md:my-3"
                            dir="rtl" name="stage">
                        <option style="color:rgb(59 59 59)" disabled selected>اختر المرحلة ...</option>
                ...
                <label>
                    <select style="background-color:#fff;"
                            id="subject_id"
                            class="form-control w-full appearance-none border-gray-200 rounded border shadow-sm p-3 my-3"
                            dir="rtl" name="subject">
                        <option style="color:rgb(59 59 59)" disabled selected>اختر المادة ...</option>
                ...
                <div class="submit">
                    <button
                        class=" border-gray-200 border rounded shadow-sm p-3  w-full bg-blue-400 text-white my-3 focus:outline-none hover:bg-blue-600"
                        type="submit">البحث عن معلم
                    </button>
                </div>
            ...
            <div class="search-number px-5 my-2">
                <p class="border-gray-200 bg-white shadow-sm rounded p-3"> إجمالي عدد المعلمين المسجلين لدينا:
            ...
            <ul class="search-result md:px-5 mt-5 ">
                ...
                @foreach($teachers as $teacher)
                    <li class="teacher mt-10 px-5 bg-white shadow-md grid grid-cols-5 gap-4 py-8">
                        <div
                            class="logo bg-white col-span-1 col-start-3 lg:col-start-1 row-span-2 md:row-span-1 flex items-center justify-center">
                            <img src="{{ asset('uploads') }}/{{$teacher->profile_img}}" alt="pic"
                                 class="h-20 lg:h-32 rounded-full shadow-sm"></div>
                        <div class="info lg:col-span-3 col-span-5 bg-white text-center">
                            <a href="{{url('teachers/show')}}/{{$teacher->id}}"
                               class="hover:text-blue-800 text-blue-500">
                                <h2 class="text-lg font-bold">{{ $teacher->user->first_name .' '. $teacher->user->last_name}}</h2>
                            </a>
                            <!-- stars -->
                            ...
                            <div class="pref mt-3">
                                ...
                                <div
                                    class="grade_exp flex flex-col md:flex-row pt-10 md:mt-3 md:items-center md:justify-around lg:justify-between">
                                    <!-- Teacher Graduation -->
                        ...
                        <div class="rate lg:col-span-1 col-span-5 bg-white flex-col items-center justify-between flex">
                            ...
                            <div class="submit">
                                <a href="{{url('booking/teacher')}}/{{$teacher->id}}">
                                    <button
                                        class=" border-gray-200 border rounded shadow-sm py-2 px-3 text-sm md:text-base w-full bg-blue-500 text-white my-3 focus:outline-none hover:bg-blue-600">
                                        احجز الآن
                                    </button>
                                </a>
                            </div>
